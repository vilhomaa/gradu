...
m_error_fractions     <- c(0.1,0.8) # What error fractions should datasets be generated with
r_error_fractions     <- c(0.1,0.8) #  be generated with
tau_error_fractions   <- c(0.1,0.8)
...
B                     <- 20 # change this number to get multiple iterations
...
for (r_error_fraction in r_error_fractions) {
  for (m_error_fraction in m_error_fractions) {
    for (tau_error_fraction in tau_error_fractions) {
      ...
      assign(
        paste0(
          "results_with_error_fractions_r_",r_error_fraction,"_m_",m_error_fraction),
        result_buffer_list)
    }
  }
}

save.image("results/results_9_fracs_10_cv.RData", compress = TRUE)
